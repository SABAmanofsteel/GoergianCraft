...
public class GorgasaliSwordItem extends Item {
    ...
    private static final double KNOCKBACK_XZ = 2.0;   // horizontal force
    private static final double KNOCKBACK_Y = 1.0;    // vertical force
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient && world instanceof ServerWorld serverWorld) {
            ...
            // Fire particles circling the player
            int FIRE_PARTICLE_COUNT = 24;
            double CIRCLE_RADIUS = 2.0;

            for (int i = 0; i < FIRE_PARTICLE_COUNT; i++) {
                double angle = (2 * Math.PI / FIRE_PARTICLE_COUNT) * i;
                double px = user.getX() + Math.cos(angle) * CIRCLE_RADIUS;
                double pz = user.getZ() + Math.sin(angle) * CIRCLE_RADIUS;
                serverWorld.spawnParticles(
                        ParticleTypes.FLAME,
                        px, user.getY() + 0.5, pz,
                        3, 0, 0, 0, 0.05
